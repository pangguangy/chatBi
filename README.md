...
基于 **Python + LangGraph + Streamlit + SQLite + DeepSeek API** 的制造业经营数据分析 Agent。
...
```
制造业运营态势ChatBI-Agent/
├── config.py              # 全局配置（DeepSeek API 密钥位置、数据/数据库路径、重试次数）
...
└── tools/
    ...
    ├── llm.py             # DeepSeek API 调用封装（OpenAI 兼容接口，urllib 实现）
...
### 1. 环境要求
- Python 3.9+
- 可访问 DeepSeek API 的密钥（离线模式也可运行，见下文）
...
### 3. 配置 DeepSeek API 密钥（可选，推荐用 .env）

复制 `.env.example` 为 `.env`，填入你自己的 Key（每个使用者填自己的即可，互不冲突）：

```bash
cp .env.example .env      # Windows 下直接复制文件并改名为 .env
```
...
> `.env` 已被 `.gitignore` 忽略，**不会**随代码提交/泄露；切勿把 Key 写进 `config.py`
> 或提交到 Git。
...
4. 在 Space 的 **Settings -> Secrets** 中添加 `DEEPSEEK_API_KEY`；
